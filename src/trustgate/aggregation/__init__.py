"""Aggregate scanner reports into Trust Gate's legacy finding format."""
...
SENSITIVE_REPORT_KEY = re.compile(
    ...
    r"authorization|credential|match)",
    re.IGNORECASE,
)
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.required_scanner:
        ...
        required_scanners = set(SCANNER_NAMES) - set(args.optional_scanner or ())
    scanner_specs = (
        (
            "bandit",
            reports_dir / "bandit_report.json",
            reports_dir / "bandit_execution.json",
            parse_bandit,
        ),
        (
            "semgrep",
            reports_dir / "semgrep_report.json",
            reports_dir / "semgrep_execution.json",
            parse_semgrep,
        ),
        (
            "pip-audit",
            reports_dir / "pip_audit_report.json",
            reports_dir / "pip-audit_execution.json",
            parse_pip_audit,
        ),
        (
            "trivy",
            reports_dir / "trivy_report.json",
            reports_dir / "trivy_execution.json",
            parse_trivy,
        ),
        (
            "gitleaks",
            reports_dir / "gitleaks_report.json",
            reports_dir / "gitleaks_execution.json",
            parse_gitleaks,
        ),
    )
