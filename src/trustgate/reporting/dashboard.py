...
def finding_tool(finding):
    return finding.get('scanner', finding.get('tool', 'Unknown'))
...
def finding_line(finding):
    return finding.get('start_line', finding.get('line'))
...
scored = [f for f in findings if f.get('confidence') is not None]
high_conf_count = len(
    [f for f in scored if finding_confidence_tier(f) == 'High']
)
noise_count = sum(
...
for f in findings:
    ...
    tool = finding_tool(f)
    line = finding_line(f)
    confidence_tier = finding_confidence_tier(f)
    conf_label, conf_class = confidence_label(f)
    rule_id = f.get('rule_id','')
    rule_short = rule_id.rsplit('.',1)[-1] if '.' in rule_id else rule_id
    finding_rows += (
        f'<tr data-tool="{esc(tool)}" data-sev="{esc(sev)}" data-conf="{esc(confidence_tier)}">'
        f'<td class="text-primary">{esc(tool)}</td>'
        f'<td>{badge(sev)}</td>'
...
if scored:
    ...
    triage_rows = ''.join(
        f'<tr>'
        f'<td class="text-primary">{esc(finding_tool(f))}</td>'
        ...
        f'<td style="text-align:right;font-weight:600;color:var(--gray-900)">{esc(str(finding_line(f) or ""))}</td>'
        f'</tr>'
        for f in high_conf
    )
